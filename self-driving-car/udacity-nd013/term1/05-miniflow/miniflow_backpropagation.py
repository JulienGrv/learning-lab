...
class Linear(Node):
    ...
    def backward(self):
        ...
        for n in self.outbound_nodes:
            ...
            self.gradients[self.inbound_nodes[2]] += np.sum(
                grad_cost, axis=0, keepdims=False)
...
class Sigmoid(Node):
    ...
    def backward(self):
        ...
        for n in self.outbound_nodes:
            ...
            """
            node = self.inbound_nodes[0]
            self.gradients[node] += grad_cost * self._sigmoid(node.value) * (
                1 - self._sigmoid(node.value))
